...
{
    public class MoldormSprite : ISprite
    {
        private int currentFrame;
        private int totalFrames;
        private Texture2D _spriteSheet;
        private int millisecondsSinceLastUpdate = 0;
        private int millisecondsPerUpdate = 60;

        Rectangle sourceRectangle;

        public MoldormSprite(Texture2D spriteSheet)
        {
        ...
        public void Draw(SpriteBatch spriteBatch, Vector2 location)
        {
            ...
            if (currentFrame == 0)
            {
                sourceRectangle = new Rectangle(140, 59, 8, 10);
            }
            else
            {
                sourceRectangle = new Rectangle(188, 59, 8, 10);
            }
